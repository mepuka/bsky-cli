...
export const deriveCommand = Command.make(
  ...
  ({ source, target, filter, filterJson, filterHelp, includeAuthor, excludeAuthor, mode, reset, yes, dryRun }) =>
    Effect.gen(function* () {
      ...
      const sourceRefResult = yield* storeOptions.loadStoreRef(source).pipe(
        Effect.either
      );
      if (sourceRefResult._tag === "Left") {
        const error = sourceRefResult.left;
        if (
          error._tag === "StoreNotFound" &&
      ...
      const sourceRef = sourceRefResult.right;
